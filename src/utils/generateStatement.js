...
import { getClientAccountSummary } from './paymentAllocation.js'
...
async function buildStatementDocInternal({ client, transactions = [], payments = [], options = {} }) {
  ...
  const typeLabel = (type) => {
    ...
    if (type === 'accountPayment') return L.accountPayment
    return type
  }
  ...
  // Whole-account totals — the same three boxes (Total / Paid / Remaining)
  // the system's statement modal shows, computed the same way: headline
  // numbers are the client's full account position, independent of the
  // date range (which only filters ledger rows above).
  const { totalInvoiced, totalPaid, balance: accountBalance } =
    getClientAccountSummary(transactions, payments, openingBalance)
  ...
  setFont('bold', 10)
  doc.text(L.totals, pw / 2, y, { align: 'center' })
  y += 5
  ...
  const boxes = [
    { label: L.totalAmount, value: summary.total, fill: [239, 246, 255], border: [191, 219, 254], valueColor: COLORS.text },
    { label: L.paidAmount, value: summary.paid, fill: [240, 253, 244], border: [187, 247, 208], valueColor: [22, 101, 52] },
    {
      label: isCredit ? L.customerCredit : L.remainingAmount,
      value: Math.abs(summary.remaining),
      fill: [254, 242, 242],
      border: [254, 202, 202],
      valueColor: isCredit ? [22, 101, 52] : [153, 27, 27],
    },
  ]
  const orderedBoxes = isAr ? [...boxes].reverse() : boxes
  orderedBoxes.forEach((box, i) => {
    const x = marginL + i * (boxW + boxGap)
    ...
    setFont('bold', 10)
    doc.text(fmtCur(box.value), x + boxW / 2, y + 15, { align: 'center' })
  })
  y += boxH
