...
6. **Gemini tool-schema 400s** (`Unknown name $schema / exclusiveMinimum / additionalProperties / "type" [...] cannot start list`) — `@modelcontextprotocol/sdk` hands the model Zod-emitted JSON Schemas containing fields that Gemini's function-declaration subset rejects. New `bot/src/gemini/schemaSanitize.ts` strips them and rewrites `type: ['x','y']` unions into `anyOf` before the wire send. Surface called from `GeminiClient.toFunctionDeclarations`.
7. **`Each Content should have at least one part`** — the in-memory session store used to push a user's voice-/photo-only turn as `{role:'user', parts:[]}` (no text-only fields). The Gemini SDK's `validateChatHistory` then crashes at the next `startChat`. Two safety nets added: the stored user turn is now guaranteed to have at least one text part (placeholder `(voice note)` etc. when only media parts were supplied); `toGeminiHistory` defensively coalesces any remaining empty user turns to `(non-text input)`.
8. **`Unknown name "response" ... Proto field is not repeating, cannot start list`** — Gemini rejects function_response payloads whose `response` value is null, an array, or a primitive at the top level (Protobuf Struct, not a list). `GeminiClient.runLoop` now coerces any non-object `responsePayload` into `{result: ...}` before posting.
9. **Telegram `can't parse entities at byte offset 515`** — the bot was sending every reply in Markdown mode. Gemini's replies freely use `*`, `_`, backticks, etc., and Telegram 400s on unterminated entities. Switched `sendText` to plain-text mode.
10. **Voice downloads silently stalling** — `downloadTelegramFile` had no fetch timeout. Wrapped the call in an `AbortController` (30s) so transient stalls surface as `'Couldn't download that voice note.'` rather than blocking the user turn.
11. **`whoami` 500'ing when bot lacks an end-user JWT** — `auth.getUser(jwt)` requires a real user JWT (the bot holds only the service-role). whoami now degrades gracefully to `admin.getUserById` (service-role friendly) when the JWT is empty or stale, and MCP's `requireAuth` preserves the linked user id even without a JWT so audit/RBAC paths stay accurate.
